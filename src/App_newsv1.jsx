...
const CRICKET_PLAYER_IMAGES = {
  Virat: 'https://img1.hscicdn.com/image/upload/f_auto,t_h_100_2x/lsci/db/PICTURES/CMS/316600/316605.jpg',
  Rohit: 'https://img1.hscicdn.com/image/upload/f_auto,t_h_100_2x/lsci/db/PICTURES/CMS/316600/316657.jpg',
  Bumrah: 'https://img1.hscicdn.com/image/upload/f_auto,t_h_100_2x/lsci/db/PICTURES/CMS/313600/313687.jpg',
  Dhoni: 'https://img1.hscicdn.com/image/upload/f_auto,t_h_100_2x/lsci/db/PICTURES/CMS/316600/316615.jpg',
}
...
function TVNewsBulletin({ matches }) {
  ...
  // Speak headline
  const speakHeadline = useCallback((text) => {
    const synth = synthRef.current;
    if (!synth) return;
    synth.cancel();
    const clean = text.replace(/[🏆🎯📈🔥👑💰🚀📉💸💎🛡️🌅🎩⚙️🐉🦁]/gu, '').trim();
    const utter = new SpeechSynthesisUtterance(`Breaking news from VOIS Panthers. ${clean}`);
    utter.rate = 0.95;
    utter.pitch = 1.05;
    utter.volume = 1;
    const voices = synth.getVoices();
    const preferred = voices.find(v => v.lang === 'en-IN') || voices.find(v => v.lang.startsWith('en') && v.name.toLowerCase().includes('female')) || voices.find(v => v.lang.startsWith('en'));
  ...
  useEffect(() => {
    if (!isMuted && headlines[currentIdx]) {
      speakHeadline(headlines[currentIdx]);
    } else {
      synthRef.current?.cancel();
      setIsSpeaking(false);
    }
  }, [currentIdx, isMuted, headlines, speakHeadline]);
  ...
  // Extract player name from headline for avatar
  ...
  // Parse emoji category
  const getCategoryInfo = (h) => {
    if (h.includes('👑') || h.includes('LEADERBOARD')) return { tag: 'LEADERBOARD', color: '#f5a623', bg: 'rgba(245,166,35,0.15)' };
    if (h.includes('🏆') || h.includes('SEASON ELITE')) return { tag: 'SEASON RECORD', color: '#ffd700', bg: 'rgba(255,215,0,0.15)' };
    if (h.includes('🔥') || h.includes('HOT STREAK')) return { tag: 'HOT STREAK', color: '#ff6b35', bg: 'rgba(255,107,53,0.15)' };
    if (h.includes('📈') || h.includes('STAT BURST')) return { tag: 'STAT BURST', color: '#2ecc71', bg: 'rgba(46,204,113,0.15)' };
    ...
    if (h.includes('🛡️') || h.includes('IRON GUARD')) return { tag: 'CONSISTENCY', color: '#1abc9c', bg: 'rgba(26,188,156,0.15)' };
    return { tag: 'UPDATE', color: '#8899bb', bg: 'rgba(136,153,187,0.1)' };
  };

  const cat = getCategoryInfo(headline);
  const cleanHeadline = headline.replace(/^[^\w₹]+ /, '').replace(/\([^)]+\)/, '').trim();
    ...
    <div className="tv-news-bulletin" style={{ position: 'relative', zIndex: 160 }}>
      ...
      <div className="tv-channel-bar">
        ...
        <div className="tv-channel-right">
          ...
          <button className="tv-mute-btn" onClick={toggleMute} title={isMuted ? 'Unmute anchor voice' : 'Mute anchor voice'}>
            {isMuted ? '🔇' : isSpeaking ? '🔊' : '🔈'}
      ...
      {!isMinimized && (
        <div className="tv-bulletin-body">
          ...
          <div className="tv-anchor-col">
            <div className="tv-anchor-frame">
              <div className="tv-anchor-avatar">
                <div className="tv-anchor-img-wrap">
                  {/* Animated anchor silhouette */}
                  <svg viewBox="0 0 80 90" width="80" height="90" className="tv-anchor-svg">
                    <defs>
                      <radialGradient id="anchorGlow" cx="50%" cy="40%" r="60%">
                        <stop offset="0%" stopColor="#f5a623" stopOpacity="0.3"/>
                        <stop offset="100%" stopColor="#0a0f1e" stopOpacity="0"/>
                      </radialGradient>
                    </defs>
                    <ellipse cx="40" cy="22" rx="16" ry="16" fill="#1b2540" stroke="#f5a623" strokeWidth="1.5"/>
                    <ellipse cx="40" cy="22" rx="12" ry="12" fill="#f5a623" opacity="0.15"/>
                    <text x="40" y="28" textAnchor="middle" fontSize="16" fill="#f5a623">👤</text>
                    <path d="M15 75 Q15 50 40 48 Q65 50 65 75" fill="#1b2540" stroke="#f5a623" strokeWidth="1.5"/>
                    <rect x="30" y="60" width="20" height="3" rx="1" fill="#f5a623" opacity="0.6"/>
                    <ellipse cx="40" cy="85" rx="25" ry="5" fill="url(#anchorGlow)"/>
                  </svg>
                </div>
                <div className="tv-anchor-name">ANCHOR</div>
                <div className="tv-anchor-title">VOIS Panthers Desk</div>
                {isSpeaking && !isMuted && (
                  <div className="tv-speaking-indicator">
                    <span/>
                    <span/>
                    <span/>
                    <span/>
          ...
          <div className="tv-news-col">
            ...
            {mentionedPlayer && (
              <div className="tv-player-spotlight">
                <div className="tv-player-avatar-wrap">
                  {avatarUrl ? (
                    <img src={avatarUrl} alt={mentionedPlayer} className="tv-player-avatar-img"
                      onError={e => { e.target.style.display='none'; e.target.nextSibling.style.display='flex'; }}
                    />
                  ) : null}
                  <div className="tv-player-avatar-fallback" style={{ display: avatarUrl ? 'none' : 'flex', background: `linear-gradient(135deg, ${COLORS[PLAYERS.indexOf(mentionedPlayer)]}, #0a0f1e)` }}>
                    {mentionedPlayer[0]}
                  </div>
                </div>
                <div className="tv-player-info">
                  <div className="tv-player-spotlight-name">{mentionedPlayer.toUpperCase()}</div>
            ...
            <div className="tv-progress-bar">
              <div className="tv-progress-fill" key={currentIdx} style={{ '--bar-color': cat.color }}/>
            </div>
          </div>
      ...
      <div className="tv-bottom-ticker">
        <div className="tv-ticker-label" style={{ background: '#c0392b' }}>BREAKING NEWS</div>
        <div className="tv-ticker-scroll">
          <div className="tv-ticker-track">
            {headlines.map((h, i) => (
              <span key={i} className="tv-ticker-item">
                <span className="tv-ticker-bullet" style={{ color: i === currentIdx ? '#f5a623' : '#c0392b' }}>◆</span> {h}
              </span>
            ))}
            {headlines.map((h, i) => (
              <span key={`d-${i}`} className="tv-ticker-item">
                <span className="tv-ticker-bullet" style={{ color: i === currentIdx ? '#f5a623' : '#c0392b' }}>◆</span> {h}
              </span>
            ))}
          </div>
