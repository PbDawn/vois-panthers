...
function MatchLogAdmin({ matches: initialMatches, onMatchesSave }) {
  ...
  // ── TransferTable (inline, uses MatchLogAdmin state/helpers) ──
  function TransferTable() {
    ...
    matches.forEach(m => {
      ...
      PLAYERS.forEach(p => {
        ...
        const isDone = typeof tVal === 'object' && tVal !== null ? tVal[p] === true : tVal === true
      ...
      if (typeof currentT === 'object' && currentT !== null) {
        ...
        PLAYERS.forEach(p => {
          ...
          if (r === 1 || (r === 2 && prizes.winnerCountLimit === 2)) {
            newT[p] = p === player ? !(currentT === true) : currentT === true
          }
        })
      }
      const newMatches = matches.map(mm => String(mm.matchno) === String(m.matchno) ? { ...mm, transferred: newT } : mm)
      setMatches(newMatches)
      await saveToCloud(newMatches)
    }
                    ...
                    <td style={{ padding: '10px 14px' }}>
                      <button
                        ...
                        style={{
                          ...
                          transition: 'all 0.15s', whiteSpace: 'nowrap', opacity: saving ? 0.6 : 1
                        }}
                      >
...
function PredictionAdmin({ matches }) {
  // Allow completed AND in-progress matches (contest matches with a date set)
  const completedMatches = matches
    .filter(m => m.contest === 'yes' && m.date)
    ...
    <div style={{ padding: '16px 20px', color: '#e8eaf6', fontFamily: "'Rajdhani', sans-serif" }}>
      ...
      {completedMatches.length === 0 ? (
        <div style={{ fontSize: 12, color: '#8899bb', padding: 24, textAlign: 'center', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: 10 }}>
          No contest matches found. Add matches with Contest = Yes in the Match Log tab first.
        </div>
      ) : (
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ fontSize: 10, color: '#8899bb', marginTop: 3 }}>
                      {m.teamwon ? `${m.teamwon} Won · ` : '🔴 In Progress · '}{participantCount} prediction participant(s)
                    </div>
                  </div>
                )
              })}
